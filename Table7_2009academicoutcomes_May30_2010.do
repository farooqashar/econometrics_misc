...
*It runs regressions to estimate 2009 academic outcome treatment effects controlling for strata (gender, year, hs grade
...
/* TABLE 7: CONTROL MEANS AND ACADEMIC OUTCOME REGRESSIONS FOR FULL YEAR, SPRING, AND FALL VARS WITH VARIOUS CONTROLS */

local depvarlist1 "earnings avggrade gpa earned ptsover70"

*Loop over academic variable groups
local tabnum=7
forvalues dvlistnum = 1(1)3{
...
*Loop over dependent variable list
foreach depvar in `depvarlist`dvlistnum'' {
	...
	*Loop over subgroups: ^F is all women, ^M is all men, 1$ is all 1st years, 0$ is all 2nd years, . is everyone
	...
	foreach strata in F_1 F_0 ^F M_1 M_0 ^M 1$ 0$ . {
		...
		foreach controlset in "`fullcontrols'" {
			qui eststo: reg `depvar'2009 T `controlset' if regexm(s_group,"`strata'"), r
		}
	}
...
*Close loop over academic variable groups
local ++tabnum
}
